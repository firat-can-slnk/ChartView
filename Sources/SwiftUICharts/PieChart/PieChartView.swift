...
public struct PieChartView : View {
    ...
    public init(data: [Double], title: String, legend: String? = nil, style: ChartStyle = Styles.pieChartStyleOne, form: ChartForm = ChartForm.medium, dropShadow: Bool? = true, valueSpecifier: String? = "%.1f"){
        self.data = data
        self.title = title
        self.legend = legend
        self.style = style
        self.formSize = form.getSize()
        if self.formSize == ChartForm.large.getSize() {
            self.formSize = ChartForm.extraLarge.getSize()
        }
